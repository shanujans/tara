...
// ============================================================
// ADDED: MCP search & quantum ranking helpers
// ============================================================
...
Tools: search_products, quote_delivery, create_order via Kapruka MCP.`;
  ...
  // Collect the full streamed response instead of piping directly.
  ...
  // ============================================================
  // MODIFIED: After streaming finishes, check for quantum search tag
  // ============================================================
  ...
  if (qMatch) {
    ...
    products = await quantumSearch(
      primary,
      alternative,
      creative,
      budget ? Number(budget) : undefined
    );
    // Remove the machine‑only tag from the visible reply
    cleanedText = fullText.replace(qMatch[0], '').trim();
  }

  // ============================================================
  // Build response with optional X-Products header
  // ============================================================
  ...
  if (products.length) {
    headers.set('X-Products', JSON.stringify(products));
  }
