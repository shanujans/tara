 ...
 * Uses a CLOSED shadow DOM so the host page's CSS never bleeds in, and the
 * host page's own scripts can't reach into TARA's DOM either.
 * State (cart, language) is stored in localStorage.
 * Product context is auto-extracted on Kapruka product pages.
 *
 * Brought to parity with chrome-extension/content.js (v2) on 2026-07-04:
 *  • Shadow root is now 'closed' (was 'open' — no isolation from host scripts)
 *  • Product-context selectors upgraded to the same 4/4/3+3 chains content.js
 *    uses (was missing a name selector, a price selector + "was" exclusion,
 *    the 5+-digit URL ID fallback, and ALL data-attribute ID fallbacks)
 *  • Added role="dialog"/aria-modal/aria-label on the panel and
 *    aria-haspopup="dialog" on the bubble (panel had no ARIA semantics at all)
 *  • Added focus management: iframe.focus() on open, bubble.focus() on close
 *    (previously keyboard users had no focus indication either way)
 * ─────────────────────────────────────────────────────────────────────────────
 */
(function () {
  'use strict';
  ...
  // gate below matched zero real Kapruka pages, so context extraction never
  // fired in production. Kept as a secondary check in case some templates differ.
  function getContext() {
    ...
    if (!name) return null;
    return { name: encodeURIComponent(name.slice(0, 100)), price: price ? encodeURIComponent(price) : '', id: id ? encodeURIComponent(id) : '' };
  }
